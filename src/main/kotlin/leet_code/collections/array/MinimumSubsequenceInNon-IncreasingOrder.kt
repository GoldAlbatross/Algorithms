 ...
 * if there still exist multiple solutions, return the subsequence with the maximum total sum of all its elements. A subsequence of an array can be obtained by erasing some (possibly zero) elements from the array.
 *
 ...
 * */
fun main() {
    println(minSubsequence(intArrayOf(4,3,8,9,10)))
...
fun minSubsequence(nums: IntArray): List<Int> {
    ...
    for (n in nums.sortedDescending()) {
        res.add(n)
        val resSum = res.sum()
        if (resSum > arrSum - resSum) return res
